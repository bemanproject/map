...
configure beman.map to fetch them automatically via CMake FetchContent.
...
Project-specific options are prefixed with `BEMAN_MAP`.
You can see the list of available options with:

```bash
cmake -LH -S . -B build | grep "BEMAN_MAP" -C 2
```
...
### `BEMAN_MAP_INSTALL_CONFIG_FILE_PACKAGE`

Enable installing the CMake config file package. Default: `ON`.
Values: `{ ON, OFF }`.

This is required so that users of `beman.map` can use
`find_package(beman.map)` to locate the library.
